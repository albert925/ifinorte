<?php
	...
	if ($idR=="") {
		echo "<script type='text/javascript'>";
			echo "alert('id tipo perfil no disponible');";
			echo "var pagina='../perfil';";
			echo "document.location.href=pagina;";
		echo "</script>";
	}
	else{
		$datos="SELECT * from tp_dic where id_tpdc=$idR";
...
<head>
	...
	<meta name="description" content="Perfil <?php echo $nmdcPf ?>" />
	<meta name="keywords" content="Financiero, prestacion de servicios" />
	<title>Perfil <?php echo "$nmdcPf"; ?>|Ifinorte</title>
...
<body>
	...
	<section>
		<article id="automargen" class="flmvycont">
			...
			<section>
				<h1><?php echo "$nmdcPf"; ?></h1>
				<?php
					$txporta="SELECT * from directivos where dc_id=$idR order by id_dic desc";
					$sql_porta=mysql_query($txporta,$conexion) or die (mysql_error());
					while ($tap=mysql_fetch_array($sql_porta)) {
						$idtap=$tap['id_dic'];
						$nmtap=$tap['nam_dic'];
						$txtap=$tap['txt_dic'];
				?>
				<article class="artport">
					<h2><?php echo "$nmtap"; ?></h2>
					<article>
						<?php echo "$txtap"; ?>
					</article>
				</article>
...
<?php
			...
			echo "<script type='text/javascript'>";
				echo "alert('Perfil ha sido eliminada o no existe');";
				echo "var pagina='../perfil';";
				echo "document.location.href=pagina;";
			echo "</script>";
		}
