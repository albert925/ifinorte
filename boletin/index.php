<?php
	...
	$Bletinesc="SELECT * from boletin where id_bl=1";
	$sql_boletin=mysql_query($Bletinesc,$conexion) or die (mysql_error());
	$numboletin=mysql_num_rows($sql_boletin);
	if ($numboletin>0) {
		while ($dt=mysql_fetch_array($sql_boletin)) {
			$idbl=$dt['id_bl'];
			$nmbl=$dt['tit_bl'];
			$xtbl=$dt['txt_bl'];
			$febl=$dt['fe_bl'];
...
<head>
	...
	<meta name="description" content="Boletin informativo de ifinorte" />
	<meta name="keywords" content="Financiero, prestacion de servicios" />
	<title>Boletín informativo|Ifinorte</title>
		...
		<article id="mn_red">
			...
			<nav id="mnP">
				<ul>
					...
					<li><a class="sell" href="../boletin">Boletin Informativo</a></li>
