<?php
	...
	if (isset($_SESSION['adm'])) {
		...
		$datos="SELECT * from boletin where id_bl=1";
		$sql_dstos=mysql_query($datos,$conexion) or die (mysql_error());
		while ($dt=mysql_fetch_array($sql_dstos)) {
			$ttbl=$dt['tit_bl'];
			$txtbl=$dt['txt_bl'];
		}
?>
...
<head>
	...
	<meta name="description" content="Administrar Boletin" />
	<meta name="keywords" content="Finacioero, prestacion de servicios" />
	<title>admin Boletín| Ifinorte</title>
...
<body>
	...
	<section>
		<h1>Boletín Informátivo</h1>
		<article id="automargen">
			<form action="nwmodif_boletin.php" method="post" class="columninput">
				<label><b>Titulo</b></label>
				<input type="text" name="nmbol" required value="<?php echo $ttbl ?>" />
				<label><b>Texto</b></label>
				<textarea id="editor1" name="txbol"><?php echo "$txtbl"; ?></textarea>
				<input type="submit" value="Modificar" />
				<script>
					CKEDITOR.replace('txbol');
				</script>
			</form>
		</article>
	</section>
