...
<head>
	...
	<meta name="description" content="Administrar Proovedor" />
	<meta name="keywords" content="Finacioero, prestacion de servicios" />
	<title>admin proveedores| Ifinorte</title>
...
<body>
	...
	<nav id="mnad">
		<a id="btB" href="#">Nuevo Proveedor</a>
	</nav>
	<section>
		<h1>Proveedores</h1>
		<article id="cjB" class="oulcajas">
			<article id="automargen">
				<form action="#" method="post" enctype="multipart/form-data" id="pvg" class="columninput">
					<label>*<b>Nombre</b></label>
					<input type="text" id="ttpv" name="ttpv" required />
					<label><b>Link</b></label>
					<input type="url" id="lkpv" name="lkpv" />
					<label><b>Imagen (resolución 230x140)</b></label>
					<input type="file" id="gmpv" name="gmpv" />
					<div id="txA"></div>
					<input type="submit" value="Ingresar" id="nvpv" />
				</form>
			</article>
		</article>
		<article id="automargen" class="flB">
			<?php
				...
				$ssql="SELECT * from proveedor order by id_prov desc";
				$rs=mysql_query($ssql,$conexion) or die (mysql_error());
				$num_total_registros= mysql_num_rows($rs);
				$total_paginas= ceil($num_total_registros / $tamno_pagina);
				$gsql="SELECT * from proveedor order by id_prov desc limit $inicio, $tamno_pagina";
			...
			?>
			<figure>
				<img src="../../../<?php echo $rtpv ?>" alt="<?php echo $nmpv ?>" />
				<figcaption class="columninput">
					<a class="doll" href="borr_proveedor.php?br=<?php echo $idpv ?>">Borrar</a>
				</figcaption>
			</figure>
			<?php
				}
